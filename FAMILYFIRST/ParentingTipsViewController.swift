...
class ParentingTipsViewController: UIViewController {
    ...
    // f_category constant for Parenting Tips
    let PARENTING_F_CATEGORY = "Ptips"
            ...
            DispatchQueue.main.async {
                ...
                case .success(let info):
                    if let data = info.data {
                        // Filter by f_category "Ptips"
                        self.feed = data.filter { feed in
                            return self.feedMatchesFCategory(feed, fCategory: self.PARENTING_F_CATEGORY)
                        }
                        
                        print("✅ Parenting Tips count: \(self.feed.count)")
                        self.tblView.reloadData()
                        
                        if self.feed.isEmpty {
                            self.showAlert(msg: "No Parenting Tips found")
                        }
                    } else {
